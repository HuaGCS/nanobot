...
- The persona detail page may expose a structured metadata summary for `PROFILE.md` / `INSIGHTS.md`. Keep it read-only, locale-backed, and based on the same `<!-- nanobot-meta: confidence=... last_verified=YYYY-MM-DD -->` contract that Dream uses; include leftover legacy `(verify)` counts for migration visibility.
...
- `agents.defaults.dream` backs the built-in Dream system job that `nanobot gateway` registers on startup. Keep `intervalH` as the canonical schedule field, accept legacy `cron` / `model` input for compatibility, and treat Dream config changes as restart-required rather than hot-reloadable.
...
- Dream memory hygiene should prefer one canonical bullet per remembered fact/pattern. Conflicting `PROFILE.md` / `INSIGHTS.md` bullets should be replaced rather than accumulated. For tentative or revalidated profile/insight bullets, prefer structured metadata comments such as `<!-- nanobot-meta: confidence=low -->` and `last_verified=YYYY-MM-DD`; legacy `(verify)` markers remain compatibility-only.
