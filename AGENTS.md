...
- `gateway.status.push` may optionally push status directly to Star-Office-UI over HTTP. When enabled, require a non-empty `officeUrl` and `joinKey`, use the upstream `join-agent` / `agent-push` contract, and treat push settings as hot-reloadable runtime config.
...
- The admin visual config should expose `gateway.status.enabled` / `gateway.status.authKey` plus `gateway.status.push.*` for Star-Office-UI integration, `tools.exec.*` for shell-tool runtime control, and common single-instance channel credential cards for `whatsapp`, `telegram`, `discord`, `feishu`, `dingtalk`, `slack`, `qq`, `matrix`, and `wecom`. If a channel is already using `instances`, keep it read-only in the visual editor and preserve raw JSON as the edit path.
