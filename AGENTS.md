...
- `gateway.status.push` may optionally push status directly to Star-Office-UI over HTTP. Keep `mode=guest` as the backward-compatible default using `join-agent` / `agent-push` with a required `joinKey`, and also support `mode=owner` for the main `/set_state` contract without a `joinKey`. Treat push settings as hot-reloadable runtime config.
